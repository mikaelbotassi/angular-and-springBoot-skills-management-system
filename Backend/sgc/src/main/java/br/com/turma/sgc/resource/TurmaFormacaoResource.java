...
public class TurmaFormacaoResource {
    ...
    public ResponseEntity<List<TurmaFormacaoDTO>> buscarContemColaborador(@PathVariable Integer idColaborador) {
        return ResponseEntity.ok().body(turmaFormacaoService.buscarContemColaborador(idColaborador));
    }

    @GetMapping("/qTFIniciada")
    public ResponseEntity<List<TurmaFormacaoDTO>> queryTurmaFormacaoIniciada() {
        return ResponseEntity.ok().body(turmaFormacaoService.queryTurmaFormacaoIniciada());
    }
